		...
		void DetectionOutputLayer<Dtype>::Forward_cpu(
			...
			for (int i = 0; i < num; ++i) {
				...
				if (need_save_) {
					...
					++name_count_;
					if ( (name_count_ % num_test_image_ == 0) || (name_count_ % 20000 == 0) ) {
						if (output_format_ == "VOC") {
							...
							int split = ceil(name_count_ / (float)20000);
							std::ostringstream ss;
							ss << split;
							boost::filesystem::path output_directory(output_directory_);
							boost::filesystem::path file(output_name_prefix_ + iter_num + "_split_" + ss.str() + ".txt");
							boost::filesystem::path out_file = output_directory / file;
							std::ofstream outfile;
							outfile.open(out_file.string().c_str(), std::ofstream::out);
							...
							// 	BOOST_FOREACH(ptree::value_type &cur_box, pt.get_child("boxes_list")) {
							...
							LOG(INFO) << "Output file written: " << out_file.string() << std::endl;
						}
						if (name_count_ % num_test_image_ == 0) {
							name_count_ = 0;
						}
